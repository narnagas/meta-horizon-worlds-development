...
import { PuzzleResetController, TimeoutDelayScheduler } from '../systems/puzzle-reset-controller';
...
export function createLobbyEscapeRoom() {
  ...
  const puzzle = new SequencePuzzle(events, 'lobby-puzzle', ['lobby-button-a', 'lobby-button-c', 'lobby-button-b']);
  ...
  const reset = new PuzzleResetController(events, puzzle, 'lobby-puzzle', new TimeoutDelayScheduler());

  const start = () => {
    for (const adapter of adapters) adapter.start();
    puzzle.start(); door.start(); light.start(); sound.start(); reset.start();
  };

  const stop = () => {
    for (const adapter of adapters) adapter.stop();
    puzzle.stop(); door.stop(); light.stop(); sound.stop(); reset.stop();
  };

  return { events, buttons: { a: buttonA, b: buttonB, c: buttonC }, puzzle, door, light, sound, reset, start, stop };
}
